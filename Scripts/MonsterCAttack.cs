		...
		if (timer>=timeBetweenAttack && collider.gameObject.tag == "Player") {
			...
			if(GameManager.gameManager==null || GameManager.gameManager.gameState==GameManager.GameState.Playing){
				...
				if (enemyAttackAudio != null) {				//在敌人位置处播放敌人的攻击音效
					...
					//CharacterDemoController characterController= GetComponent<CharacterDemoController> ();
				GameObject player = GameObject.FindGameObjectWithTag("Player");
					CharacterDemoController c = player.gameObject.GetComponent<CharacterDemoController> ();
				if (c != null) {
