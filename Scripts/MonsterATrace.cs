...
public class MonsterATrace : MonoBehaviour {
	public GameObject target;		//敌人的追踪目标
	public float moveSpeed=8.0f;	//敌人的移动速度
	public float minDist=2.5f;		//追踪距离，当敌人与目标的距离小于等于该值时，敌人不再追踪目标

	private float dist;				
	private Animator animator;				//敌人的Animator组件
	private MonsterAHealth monsterAHealth;		//敌人的生命值脚本
	...
	void Start () {
		...
		monsterAHealth = GetComponent<MonsterAHealth> (); //获取敌人的生命值脚本
		target = GameObject.FindGameObjectWithTag("Player");
	}

	//每帧执行一次，用于敌人追踪目标
	void Update () {
		if (monsterAHealth!=null && monsterAHealth.health <= 0) return;	//当敌人死亡时，敌人无法追踪目标
		if (target == null) {					//当追踪目标未设置时，敌人无法追踪目标
			animator.SetBool ("isWalk", false);	//设置动画参数，将布尔型参数isStop设为true：敌人未追踪目标，播放停驻动画
			return;
		}
		...
		if (GameManager.gameManager !=null && GameManager.gameManager.gameState == GameManager.GameState.Playing  ) {	
			if (GameManager.gameManager.player.WeaponState != 2 && GameManager.gameManager.player.heroList [0] != true) {
			...
			if (dist >= TraceRange || GameManager.gameManager.player.WeaponState == 2 || GameManager.gameManager.player.heroList [0] == true) {
